...
from parsing_job.database.schemas import RawListingHtml
...
def get_scraped_data(
    db_session: Session,
    acceptable_object_types: list[str],
) -> dict[str, list[RawListingHtml]]:
    """Get the scraped data grouped by object type."""
    scraped_data = (
        db_session.execute(
            select(RawListingHtml).where(
                RawListingHtml.object_type.in_(acceptable_object_types),
            ),
        )
        .scalars()
        .all()
